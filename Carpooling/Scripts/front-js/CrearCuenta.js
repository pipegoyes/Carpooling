$(document).ready(function () {
    //avance y retroceso entre los paso del registro
    $("[id*=btnSiguientePaso1]").click(function () {
        var flag = window.Page_ClientValidate('ValidacionPaso1');
        if (flag) {
            $("[id*=divPaso1Registro]").slideToggle(1000, null);
            $("[id*=divPaso2Registro]").fadeIn("slow", null);
        }
    });

    $("[id*=btnSiguientePaso2]").click(function () {
        var flag = window.Page_ClientValidate('ValidacionPaso2');
        if (flag) {
            $("[id*=divPaso2Registro]").slideToggle(1000, null);
            $("[id*=divPaso3Registro]").fadeIn("slow", null);
        }
    });

    $("[id*=btnAtrasPaso2]").click(function () {
        $("[id*=divPaso1Registro]").slideToggle(1000, null);
        $("[id*=divPaso2Registro]").fadeOut("slow", null);
    });

    $("[id*=btnAtrasPaso3]").click(function () {
        $("[id*=divPaso2Registro]").slideToggle(1000, null);
        $("[id*=divPaso3Registro]").fadeOut("slow", null);
    });
...
function ObtenerDiasMes() {
    ...
    if (ddlAnioNacimiento.val() != '-1' && ddlMesNacimiento.val() != '-1') {
        $.ajax({
            type: "POST",
            url: "CrearCuenta.aspx/ObtenerDiasDdl",
